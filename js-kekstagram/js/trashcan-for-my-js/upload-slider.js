...
// модификаторы классов для ползунка в css = = = =   .noUi-handle--min для сгругления левой части,  noUi-handle--max для скругления правой части
  ...
  image.style.filter = filterCssValue;
...
effectList.addEventListener('change', (evt) => {
  ...
  image.className = ''; // присваиваем имени класса пустую строку тем самым удаляем все классы если и был уже присвоен клас другого эффекта он удалится
  image.classList.add(`effects__preview--${CURRENT_FILTER}`); // добавляем неоходимый класс методом составления шаблонной строки и добавляя имя фильра
